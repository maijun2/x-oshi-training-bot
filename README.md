...
- レビュー猶予はスロットまでの時間です。**スロットは Lambda の各実行（10:00 / 13:00 / 18:00 JST、EventBridge Scheduler の
  flexible window で最大 +15 分）の 60 分後に置く**運用にしています（2026-09-12 設定、全曜日共通）:

  | Lambda 実行 | スロット | 猶予（最悪ケース） |
  |---|---|---|
  | 23:58（日報＋夜間の推し投稿検知） | 翌 08:00 | 一晩 |
  | 10:00〜10:15 | 11:15 | 60 分以上 |
  | 13:00〜13:15 | 14:15 | 60 分以上 |
  | 18:00〜18:15 | 19:15 | 60 分以上 |

  日報とレベルアップ告知は X API 直投稿で Buffer を通りません。Buffer に載るのは推し投稿への引用だけです
- スロット数/日（4）> `BUFFER_DAILY_CAP`（3）にしておくとキューが滞留しません（この不等式は崩さないこと）
