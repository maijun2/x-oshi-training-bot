...
deploy 後に Runtime を invoke し、2 タスク（react / reply_response）の応答と所要時間を表示する。
...
def main() -> int:
    ...
    parser.add_argument("--text", help="--task と併用。react の post_content / reply_response の reply_text")
    ...
    if args.runs > 1 and actions:
        ...
        print(f"react: parsed {total}/{total + failed}, actions={dict(actions)}")
