...
deploy 後に Runtime を 1 回 invoke し、3 タスクの応答と所要時間を表示する。
...
使い方:
    ...
    uv run python scripts/test_brain_invoke.py --task classify_emotion --text "嬉しいｲﾓ🍠"
"""
import argparse
import os
import sys
...
SAMPLE_TASKS = [
    ("oshi_response", {"post_content": "今日は生誕祭ライブでした！みんな来てくれてありがとう🍠", "post_type": "oshi"}),
    ("classify_emotion", {"response_text": "ジュリさん生誕祭おめでとうｲﾓ🍠✨ #さつまいもの民 #びっくえんじぇる"}),
    ("reply_response", {"reply_text": "いも丸くん今日もかわいいね", "reply_username": "fan_taro",
                        "bot_tweet_text": "ジュリさんのライブ最高だったｲﾓ🍠 #さつまいもの民 #びっくえんじぇる"}),
]
...
def main() -> int:
    ...
    parser.add_argument("--text", help="--task と併用。oshi_response の post_content / classify_emotion の response_text")
    ...
    if args.task:
        ...
        if args.text:
            key = "post_content" if args.task == "oshi_response" else "response_text" if args.task == "classify_emotion" else "reply_text"
            task_input[key] = args.text
